...
def visualize_train_process(data, models, device):
    ...
    human_map = torch.cat(
        [premap.to(device), denseposemap.to(device), posemap.to(device)], 1
    )

    dense_preserve = (
        torch.cat([dense_preserve_mask, dense_preserve_mask, dense_preserve_mask], 1)
        * 2
        - 1
    )
    preserve_map = torch.cat(
        [preserve_region[0].to(device), dense_preserve[0].to(device)], 2
    )

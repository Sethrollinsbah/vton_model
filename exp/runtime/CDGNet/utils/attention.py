...
class CDGAttention(nn.Module):
    def __init__(
        ...
        self.conv_upDim1 = nn.Sequential(
            nn.Conv1d(
                feat_out, feat_in, kSize, stride=1, padding=kSize // 2, bias=True
            ),
            nn.Sigmoid(),
        )
    ...
    def forward(self, fea):
        ...
        fea_hup = F.interpolate(
            fea_hup, (h, w), mode="bilinear", align_corners=True
        )  # n,c,h,w
        fea_wup = F.interpolate(
            fea_wup, (h, w), mode="bilinear", align_corners=True
        )  # n,c,h,w
...
class StatisticAttention(nn.Module):
    def __init__(self, fea_in, fea_out, num_classes):
        ...
        self.conv_pred = nn.Sequential(
            nn.Conv2d(
                num_classes, 1, 3, stride=1, padding=1, bias=True
            ),  # kernel size 1
            nn.Sigmoid(),
        )

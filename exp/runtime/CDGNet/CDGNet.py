...
class Decoder_Module(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(
                256, 48, kernel_size=3, stride=1, padding=1, dilation=1, bias=False
            ),
            InPlaceABNSync(48),
        )
        self.conv3 = nn.Sequential(
    ...
    def PCM(self, cam, f):
        n, c, h, w = f.size()
        cam = F.interpolate(cam, (h, w), mode="bilinear", align_corners=True).view(
            n, -1, h * w
        )
        f = f.view(n, -1, h * w)
    ...
    def forward(self, xt, xl, xPCM=None):
        ...
        with torch.no_grad():
            xM = F.relu(x.detach())
        xPCM = F.interpolate(
            self.PCM(xM, xPCM), size=(h, w), mode="bilinear", align_corners=True
        )
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.f9 = nn.Sequential(
            nn.Conv2d(
                256 + 256 + 3, 256, kernel_size=1, padding=0, dilation=1, bias=False
            ),
            InPlaceABNSync(256),
        )
        # ===============================================================
        ...
        self.L = nn.Conv2d(
            1024, num_classes, kernel_size=1, padding=0, dilation=1, bias=True
        )
        # ================================================================

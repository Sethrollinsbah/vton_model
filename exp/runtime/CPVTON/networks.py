...
class TpsGridGen(nn.Module):
    def __init__(
        ...
        self.grid_X = (
            torch.FloatTensor(self.grid_X).unsqueeze(0).unsqueeze(3).to(device)
        )
        self.grid_Y = (
            torch.FloatTensor(self.grid_Y).unsqueeze(0).unsqueeze(3).to(device)
        )
        ...
        if use_regular_grid:
            ...
            self.P_X = (
                P_X.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0, 4).to(device)
            )
            self.P_Y = (
                P_Y.unsqueeze(2).unsqueeze(3).unsqueeze(4).transpose(0, 4).to(device)
            )
...
class GMM(nn.Module):
    ...
    def __init__(self, fine_height, fine_width, grid_size, device):
        ...
        self.gridGen = TpsGridGen(
            fine_height, fine_width, device=device, grid_size=grid_size
        )

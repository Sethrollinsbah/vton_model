...
class Simple_Generator(nn.Module):
    def __init__(self, opt):
        ...
        for i in range(len(self.channels) - 1):
            self.body.append(
                ResnetBlock(
                    self.channels[i] + opt.z_dim + semantic_nc,
                    self.channels[i + 1],
                    opt,
                )
            )
        ...
        self.fc = nn.Conv2d(
            semantic_nc + self.opt.z_dim, self.channels[0], 3, padding=1
        )
        self.conv_img = nn.Conv2d(
            self.channels[-1] + opt.z_dim + semantic_nc, 3, 3, padding=1
        )
    ...
    def forward(self, seg, z=None):
        ...
        _seg = F.interpolate(
            seg, scale_factor=scale, recompute_scale_factor=False, mode="nearest"
        )
    ...
    def transform_cloth(self, seg, C_t):
        if self.bpgm is not None:
            with torch.no_grad():
                ...
                C_t = F.grid_sample(
                    C_t, grid, padding_mode="border", align_corners=True
                )
    ...
    def transform_cloth_old(self, agnostic, C_t):
        if self.bpgm is not None:
            with torch.no_grad():
                # grid, _ = self.bpgm(torch.cat((I_m, seg), dim=1), C_t)
                if self.bpgm.resolution != self.opt.img_size:
                    agnostic = F.interpolate(
                        agnostic, size=self.bpgm.resolution, mode="nearest"
                    )
                    _C_t = F.interpolate(
                        C_t,
                        size=self.bpgm.resolution,

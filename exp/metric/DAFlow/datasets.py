...
class ShapeDataset_T(data.Dataset):
    def __init__(self, opt):
        ...
        self.image_ids = np.genfromtxt(
            osp.join(opt.dataset_dir, opt.dataset_list), dtype=np.str
        )
    ...
    def __getitem__(self, index):
        ...
        source_id = (
            source_id.decode("utf-8") if isinstance(source_id, bytes) else source_id
        )
        ...
        img_source = transforms.Resize(
            (self.load_height, self.load_width), interpolation=2
        )(img_source)

        img_target = Image.fromarray(np.uint8(img_target))
        img_target = transforms.Resize(
            (self.load_height, self.load_width), interpolation=2
        )(img_target)
        ...
        pose_target[1, :, :] = 0
        pose_target = self.obtain_camera_pose(pose_target)
        pose_source = transforms.Resize(
            (self.load_height, self.load_width), interpolation=2
        )(pose_source)
        pose_target = transforms.Resize(
            (self.load_height, self.load_width), interpolation=2
        )(pose_target)
...
class MPVDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for key in self.c_names:
            ...
            c[key] = Image.open(
                osp.join(self.data_path, "images", c_name[key])
            ).convert("RGB")
            c[key] = transforms.Resize(self.load_width, interpolation=2)(c[key])
            cm[key] = Image.open(
                osp.join(
                    self.data_path, "images", c_name[key].replace(".jpg", "_mask.jpg")
                )
            )
        ...
        if random.random() > 0.2:
            img_agnostic = Image.open(
                osp.join(self.data_path, "img_agnostic", img_name)
            )
        else:
            img_agnostic = Image.open(
                osp.join(self.data_path, "img_agnostic_maskmore", img_name)
            )
        img_agnostic = transforms.Resize(self.load_width, interpolation=2)(img_agnostic)
        try:
            img = self.transform(img)
...
class FASHIONVDataset_Test(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pose_source = Image.open(
            img_source_name.replace("test_frames", "test_keypoints_noise").replace(
                "png", "jpg"
            )
        )
        pose_source = transforms.Resize(
        ...
        pose_target = Image.open(
            img_target_name1.replace("test_frames", "test_keypoints").replace(
                "png", "jpg"
            )
        )
        pose_target = transforms.Resize(
...
class FASHIONVDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        pose_source = Image.open(
            img_source_name.replace("train_frames", "train_keypoints_noise").replace(
                "png", "jpg"
            )
        )
        pose_source = transforms.Resize(
        ...
        pose_target = Image.open(
            img_target_name.replace("train_frames", "train_keypoints").replace(
                "png", "jpg"
            )
        )
        pose_target = transforms.Resize(

...
class Pix2PixHDModel(BaseModel):
    ...
    def cross_entropy2d(self, input, target, weight=None, size_average=True):
        ...
        if h != ht or w != wt:
            input = F.interpolate(
                input, size=(ht, wt), mode="bilinear", align_corners=True
            )
    ...
    def encode_input(self, label_map, clothes_mask, all_clothes_label):
        ...
        input_label = torch.cuda.FloatTensor(
            torch.Size(oneHot_size), device=self.device
        ).zero_()
        ...
        c_label = torch.cuda.FloatTensor(
            torch.Size(oneHot_size), device=self.device
        ).zero_()
    ...
    def forward(
        ...
        arm1_mask = torch.FloatTensor((label.cpu().numpy() == 11).astype(np.float)).to(
            self.device
        )
        arm2_mask = torch.FloatTensor((label.cpu().numpy() == 13).astype(np.float)).to(
            self.device
        )
        ...
        G1_in = torch.cat(
            [pre_clothes_mask, clothes, all_clothes_label, pose, self.gen_noise(shape)],
            dim=1,
        )
        ...
        CE_loss = (
            self.cross_entropy2d(
                arm_label, (label * (1 - clothes_mask)).transpose(0, 1)[0].long()
            )
            * 10
        )

        armlabel_map = generate_discrete_label(
            arm_label.detach(), 14, False, device=self.device
        )
        dis_label = generate_discrete_label(arm_label.detach(), 14, device=self.device)
        G2_in = torch.cat(
            [pre_clothes_mask, clothes, dis_label, pose, self.gen_noise(shape)], 1
        )
        ...
        G_in = torch.cat(
            [img_hole_hand, dis_label, fake_c, skin_color, self.gen_noise(shape)], 1
        )

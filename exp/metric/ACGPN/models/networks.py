...
class GANLoss(nn.Module):
    ...
    def get_target_tensor(self, input, target_is_real):
        ...
        if target_is_real:
            ...
            if create_label:
                real_tensor = self.Tensor(input.size(), device=self.device).fill_(
                    self.real_label
                )
            ...
            if create_label:
                fake_tensor = self.Tensor(input.size(), device=self.device).fill_(
                    self.fake_label
                )
...
class ConvEncoder(nn.Module):
    def __init__(
        ...
        for i in range(n_downsampling):
            ...
            model += [
                nn.Conv2d(
                    ngf * mult, ngf * mult * 2, kernel_size=3, stride=stride, padding=1
                ),
...
class AttGenerator(nn.Module):
    def __init__(
        self, output_nc, ngf=32, n_blocks=4, n_downsampling=4, padding_type="reflect"
    ):
        ...
        for i in range(n_blocks):
            model += [
                ResnetBlock(ngf * mult * 2, norm_type="in", padding_type=padding_type)
            ]
...
class STNNet(nn.Module):
    ...
    def __init__(self):
        ...
        assert (
            r1 < 1 and r2 < 1
        )  # if >= 1, arctanh will cause error in BoundedGridLocNet

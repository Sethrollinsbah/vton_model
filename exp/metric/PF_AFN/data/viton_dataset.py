...
class LoadVITONDataset(Dataset):
    ...
    def __getitem__(self, index: int) -> dict:
        ...
        # Person image
        img = Image.open(Path(self.dataroot) / f"{self.phase}_img" / im_name).convert(
            "RGB"
        )
        ...
        # Clothing image
        cloth = Image.open(
            Path(self.dataroot) / f"{self.phase}_color" / c_name
        ).convert("RGB")
        # cloth = cloth.resize((self.width, self.height))
        cloth_tensor = self.transform_image(cloth)  # [-1,1]

        # Clothing edge
        cloth_edge = Image.open(
            Path(self.dataroot) / f"{self.phase}_edge" / c_name
        ).convert("L")
        ...
        if self.phase == "train":
            ...
            un_cloth = Image.open(
                Path(self.dataroot) / f"{self.phase}_color" / un_c_name
            ).convert("RGB")
            ...
            parse_path1 = (
                Path(self.dataroot)
                / f"{self.phase}_label"
                / f"{Path(im_name).stem}.jpg"
            )
            parse_path2 = (
                Path(self.dataroot)
                / f"{self.phase}_label"
                / f"{Path(im_name).stem}.png"
            )
            ...
            dense_mask = np.load(
                Path(self.dataroot)
                / f"{self.phase}_densepose"
                / f"{Path(im_name).stem}.npy"
            ).astype(np.float32)

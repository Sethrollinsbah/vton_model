...
def train_batch(
    ...
    pre_clothes_edge_un = torch.FloatTensor(
        (edge_un.detach().numpy() > 0.5).astype(np.int64)
    )
    ...
    for num in range(5):
        ...
        if num >= 2:
            ...
            flow_sup_loss = (
                torch.norm(flow_sup_all[num].detach() - flow_all[num], p=2, dim=1)
                * weight_all
            ).sum() / (h1 * w1 * num_all)
    ...
    loss_l1_skin = criterionL1(
        p_rendered * skin_mask, skin_mask * real_image.to(device)
    )
    loss_vgg_skin = criterionVGG(
        p_rendered * skin_mask, skin_mask * real_image.to(device)
    )
    ...
    if global_step % sample_step == 0:
        ...
        e = torch.cat(
            [skin_mask.to(device), skin_mask.to(device), skin_mask.to(device)], 1
        )
        f = warped_cloth
        g = p_rendered
        ...
        i = p_tryon
        combine = torch.cat(
            [a[0], b[0], c[0], d[0], e[0], f[0], g[0], h[0], i[0]], 2
        ).squeeze()
...
def train_pf_e2e(opt):
    ...
    print_log(
        log_path, f"Load pretrained parser-based warp from {opt.pb_warp_checkpoint}"
    )
    pb_gen_model = ResUnetGenerator(8, 4, 5, ngf=64, norm_layer=nn.BatchNorm2d).to(
        device
    )
    pb_gen_model.eval()
    ...
    print_log(
        log_path, f"Load pretrained parser-based gen from {opt.pb_gen_checkpoint}"
    )
    ...
    warp_scheduler = MyLRScheduler(
        warp_optimizer, last_epoch, opt.niter, opt.niter_decay, False
    )
    gen_scheduler = MyLRScheduler(
        gen_optimizer, last_epoch, opt.niter, opt.niter_decay, False
    )
    ...
    if validate:
        ...
        val_loader = DataLoader(
            val_data, batch_size=1, shuffle=False, num_workers=opt.workers
        )
    ...
    for epoch in range(start_epoch, epoch_num + 1):
        ...
        for idx, data in enumerate(
            ...
            if global_step % opt.print_step == 0:
                eta_sec = (
                    (epoch_num + 1 - epoch) * len(train_loader) - idx - 1
                ) * eta_meter.avg
        ...
        if validate and best_fid == fid:
            ...
            print_log(
                log_path,
                "Save best with fid %.3f at epoch %d, iters %d"
                % (fid, epoch, global_step - 1),
            )
        if epoch % opt.save_period == 0:
            ...
            print_log(
                log_path,
                "Save the model at the end of epoch %d, iters %d"
                % (epoch, global_step - 1),
            )
